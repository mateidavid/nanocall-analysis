...
Nanocall is designed to work with 6-mer pore models. By default we use the models provided by Metrichor. These are 3 models, 1 for the template strand and 2 for the complement strand. Emission probabilities are calculated by multiplying the probability density from the Gaussian and Inverse Gaussian distributions that model the event mean and event standard deviation, respectively. This is done after the models are scaled (see Scaling below.)
